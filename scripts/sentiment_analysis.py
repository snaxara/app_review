...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(description="Análise de sentimento (BERTweet) sobre CSV de avaliações.")
    parser.add_argument("--input", default=CSV_PATH, help="CSV de entrada (padrão: app_review_dataset.csv)")
    parser.add_argument("--output", default=OUTPUT_CSV_PATH, help="CSV de saída (padrão: sentiment_results.csv)")
    cli = parser.parse_args()
    in_path, out_path = cli.input, cli.output
    ...
    # Carrega os dados
    print(f"\nCarregando dados de: {in_path}")
    df_reviews = load_data(in_path)
    ...
    if df_reviews is not None and not df_reviews.empty:
        ...
        if df_results is not None:
            # Salva os resultados
            print(f"\nSalvando resultados...")
            df_results.to_csv(out_path, index=False, sep=';', encoding='utf-8')
