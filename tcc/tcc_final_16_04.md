...
Em muitas soluções de IA, o contexto vem de trechos de texto guardados de forma estática (ideia próxima ao RAG). Neste trabalho, o eixo é outro: registrar fatos em grafo que acompanha o ciclo de vida do produto. Um problema típico de certa versão pode deixar de sê-lo na seguinte (Fan et al., 2024), o que ajuda a enxergar regressões e mudanças ao longo do tempo.
...
No segundo estágio, apenas os comentários negativos seguem para o GPT-4o, que extrai entidades (reconhecimento de entidades adaptado ao domínio). Para respostas mais estáveis, usou-se temperatura 0,1 e saída em JSON, na primeira passagem e na reflexão. Exemplos no prompt (“few-shot”) orientam o modelo na tarefa (Fulford & Ng, 2023). O fluxo, em ordem: extração inicial; reflexão para recuperar o que faltou na primeira leitura (por exemplo, menções implícitas); filtragem de entidades genéricas; mapeamento para BusinessCapabilities com regras híbridas; gravação no grafo temporal no Neo4j. O ganho da reflexão é comentado em Resultados e Discussões.
...
Em ambiente corporativo, o passo seguinte natural é cruzar o retrato do grafo com defeitos internos, sem expor números neste texto. Conceitualmente: priorizar onde loja e registro interno coincidem; ampliar testes nas capacidades mais “quentes”; acompanhar se melhorias reduzem defeitos ao longo das releases; e, antes de um “deploy”, focalizar regressão no que o grafo indica como tocado, em vez de sempre rodar o pacote completo. Esse uso institucional não foi medido aqui; cabe à governança de dados da empresa.
...
A Tabela 1 resume o processamento: 100 comentários negativos geraram 103 episódios (um texto pode virar mais de uma issue) e 85 issues distintas. Os 91 vínculos Issue–BusinessCapability para 85 issues fecham assim: 79 issues com uma capacidade e 6 issues com duas capacidades ao mesmo tempo (79 + 6 × 2 = 91). Não é erro de contagem; em aplicativo bancário é comum o relato atravessar mais de uma frente (acesso, segurança, experiência). O grafo explicita isso em vez de forçar um único rótulo. O mapeamento ainda merece validação humana, mas, para priorização, o desenho ajuda quando um sintoma exige coordenação entre áreas. As issues distribuem-se por 15 capacidades.
...
Com 100 comentários públicos do aplicativo CAIXA, mostrou-se viável transformar avaliações de loja em insumos para priorizar backlog, como complemento aos testes anteriores ao “deploy”. A extração gerou 85 issues; o mapeamento para BusinessCapabilities atingiu 63,6% de acurácia em texto informal; Performance e Estabilidade concentrou 23,53% das issues (20 de 85), em linha com o que mais incomoda o usuário.

A triagem automática eliminou 13,1% de negativos genéricos e reduziu custo. A reflexão recuperou cerca de 5% a 10% de entidades que a primeira passagem perdia. O grafo temporal liga versão, problema e capacidade; a modelagem admite novos tipos de nó ou relação sem reprocessar tudo de uma vez. Na apresentação para a banca, vale fixar o par de 91 vínculos e 85 issues: não é inconsistência, é registro de reclamações que cruzam mais de uma capacidade, o que no banco é típico quando o sintoma não tem um único “dono” de backlog.

A proposta ganha escala quando o feedback real entra em rotina de QA, com mais previsibilidade do que caçar avaliação em planilha. Para quem precisa justificar prioridade em comitê, o grafo oferece vocabulário comum entre produto, desenvolvimento e testes: dá para mostrar por que uma capacidade pede atenção e como isso conversa com a voz do usuário nas lojas, e, no uso interno, com defeitos registrados, sempre respeitando confidencialidade.

Há limitações. Cem comentários demonstram o método, mas não permitem generalização populacional; o uso do pipeline em janelas longas de tempo ainda merece validação; e o fluxo sequencial com GPT-4o pode virar gargalo se o volume crescer. Parte dos mapeamentos ainda pede validação com especialistas de negócio. Próximos passos naturais: ampliar amostra, revisar o grafo com produto e TI, ganhar desempenho com paralelismo e cache, padronizar modelo de documento de priorização e acompanhar o processo com métricas de monitoramento, em linha com melhoria contínua em qualidade de software. Em ambiente controlado, testar cruzamento com base interna de defeito e regressão focada pelo relacionamento versão–funcionalidade–issue, medindo ganho de eficiência quando fizer sentido para o negócio, sem violar sigilo.

Quanto à reprodutibilidade, o manual do curso não exige, por si só, link público de repositório: inclua apenas se a coordenação ou a orientadora pedirem explicitamente. Scripts e consultas ao grafo podem permanecer sob repositório privado ou serem disponibilizados sob solicitação, desde que não violem chaves de API, políticas da empresa ou a LGPD.
