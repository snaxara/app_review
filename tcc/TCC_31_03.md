...
Ensuring quality in banking applications required identifying failures that emerged in production, often invisible to traditional testing. Public app stores contained large volumes of unstructured reviews that remained underused. This work developed a quality-testing approach that operated beyond the traditional deployment pipeline, combining a temporal Knowledge Graph and artificial intelligence to automate issue extraction, link problems to Business Capabilities, and produce backlog prioritization artifacts. The methodology used a hybrid architecture: a Transformer model (BERTweet, XLM-RoBERTa) for sentiment polarity and GPT-4o for entity extraction, with storage in Neo4j. Processing real reviews of the CAIXA app showed high linkage coverage between technical issues and business areas (excluding the “Other Features” bucket). About one quarter of classified issues fell under “Performance and Stability” (23.53%), indicating that textual feedback could be turned into actionable prioritization input for engineering and quality teams. The business-capability and version structure remained amenable, under corporate confidentiality, to future integration with internal defect records and risk-based test scoping, not empirically quantified here.
...
O foco da abordagem está na análise de dados recentes coletados diretamente em produção, permitindo que equipes de qualidade identifiquem problemas de forma proativa e produzam documentos de priorização que orientem tanto a correção de "bugs" quanto a melhoria contínua dos aplicativos. A capacidade de identificar "bugs" por versão através de consultas no grafo e gerar documentos estruturados automaticamente representa um avanço considerável quando comparada às abordagens tradicionais, que dependem de análise manual de "logs", "tickets" e avaliações não estruturadas. O objetivo geral consistiu em estruturar, implementar e validar empiricamente esse pipeline com dados reais do aplicativo CAIXA, avaliando a classificação de polaridade, o mapeamento de entidades para "BusinessCapabilities" e o uso combinado de grafo temporal e métricas de SNA como suporte à priorização de backlog.

## Material e métodos

Tratou-se de pesquisa aplicada, com ênfase em evidências quantitativas e em fluxo reprodutível, voltada a apoiar testes de qualidade além da esteira clássica de desenvolvimento. Priorizou-se a identificação de "bugs" vinculados a versões específicas e a geração de artefatos que orientem a priorização do backlog em aplicativos bancários móveis. A metodologia organizou-se em seis etapas sequenciais: coleta de dados; análise da polarização dos comentários; extração de entidades; modelagem do "Knowledge Graph"; aplicação de métricas de "Social Network Analysis"; e geração dos documentos de priorização. A abordagem fundamentou-se em literatura sobre qualidade de software em contextos ágeis (Oliveira, 2014) e práticas de teste em empresas brasileiras (Nascimento, 2005), adaptada ao cenário de aplicativos financeiros. Incorporaram-se técnicas de "Social Network Analysis" (Pinheiro, 2011; Newman, 2010) para análise estrutural do "Knowledge Graph" além da contagem simples, de modo a evidenciar padrões e nós influentes na rede de problemas.
...
Pipeline de processamento de avaliações, da coleta à geração de documentos de priorização
...
A aplicação de métricas de "Social Network Analysis" ao "Knowledge Graph" construído permite identificar padrões estruturais que não seriam evidentes através de análises simples de contagem. A Figura 10 apresenta os valores de "Degree Centrality" tanto para "Issues" quanto para "BusinessCapabilities", oferecendo uma visão sobre quais entidades possuem mais conexões na rede. A Figura 11, por sua vez, combina múltiplas métricas "SNA", incluindo a densidade da rede, a distribuição de "Degree Centrality" e o ranking das 10 "Issues" com maior "Clustering Coefficient".
...
Os resultados vêm com ressalvas. Os 100 comentários bastam para demonstrar a ideia, mas não para generalizar como estudo populacional; o recorte bi-temporal longo ainda não foi validado; e o "pipeline" sequencial ao "GPT-4o" pode virar gargalo se o volume explodir. Alguns mapeamentos ainda pedem validação com quem conhece o negócio. Para frente, faz sentido ampliar a amostra, validar o grafo com produto e desenvolvimento, ganhar desempenho com paralelismo e "cache", padronizar modelos de documento para priorização, aprofundar análises por versão e acompanhar o sistema com métricas de monitoramento, em continuidade a práticas consolidadas de melhoria de processo em qualidade de software. Vale experimentar, em ambiente controlado, o cruzamento com bases internas de defeito e a leitura de densidade ao longo das releases, sempre sem comprometer sigilo, e pilotar regressão direcionada a partir dos relacionamentos entre versão, funcionalidade e "issue", medindo ganho de eficiência em ciclo de testes onde fizer sentido para o negócio.
